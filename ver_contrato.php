...
$stmt = $conn->prepare("
    SELECT c.*,
           ...
           c.no_whatsapp   as cliente_whatsapp,
        ...
        <div class="co-card-body">
            <div class="info-grid-2">
                ...
                <div class="iib">
                    ...
                    <div class="iib-value"><?php echo htmlspecialchars($contrato['cliente_telefono1'] ?? '—'); ?></div>
                </div>
                <div class="iib">
                    <div class="iib-label">Teléfono 2</div>
                    <div class="iib-value"><?php echo htmlspecialchars($contrato['cliente_telefono2'] ?? '—'); ?></div>
                </div>
                <div class="iib">
                    <div class="iib-label">No. WhatsApp</div>
                    <div class="iib-value"><?php echo htmlspecialchars($contrato['cliente_whatsapp'] ?? '—'); ?></div>
                </div>
                <div class="iib">
                    <div class="iib-label">Cobrador</div>
                    <div class="iib-value"><?php echo htmlspecialchars($contrato['cobrador_nombre'] ?? '—'); ?></div>
                </div>
                <div class="iib full">
                    <div class="iib-label">Dirección</div>
                    <div class="iib-value"><?php echo htmlspecialchars($contrato['cliente_direccion'] ?? '—'); ?></div>
                </div>
                <?php if (!empty($contrato['ubicacion_ref'])): ?>
                <div class="iib full">
                    <div class="iib-label"><i class="fas fa-map-marker-alt"></i> Ref. Ubicación</div>
                    <div class="iib-value"><?php echo htmlspecialchars($contrato['ubicacion_ref']); ?></div>
                </div>
                <?php endif; ?>
            </div>
        </div>
...
<?php if (!empty($contrato['ubicacion_lat']) && !empty($contrato['ubicacion_lng'])): ?>
<!-- ============================================================
...
<div class="co-card fade-in delay-2">
    <div class="co-card-header">
        <div class="co-card-title">
            <div class="co-card-icon" style="background:#EFF6FF;color:#1D4ED8;">
                <i class="fas fa-map-marker-alt"></i>
            </div>
            <div>
                <div class="co-title-text">Ubicación del Contrato</div>
                <div class="co-title-sub">Localización geográfica registrada</div>
            </div>
        </div>
    </div>
    <div class="co-card-body" style="padding:12px;">
        <div id="mapaVerContrato"
             style="height:280px;border-radius:8px;border:1px solid #e2e8f0;"></div>
        <div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap;">
            <a href="https://www.google.com/maps?q=<?php echo $contrato['ubicacion_lat']; ?>,<?php echo $contrato['ubicacion_lng']; ?>"
               target="_blank" class="btn btn-secondary btn-sm">
                <i class="fas fa-map"></i> Abrir en Google Maps
            </a>
            <a href="https://waze.com/ul?ll=<?php echo $contrato['ubicacion_lat']; ?>,<?php echo $contrato['ubicacion_lng']; ?>&navigate=yes"
               target="_blank" class="btn btn-secondary btn-sm">
                <i class="fas fa-route"></i> Abrir en Waze
            </a>
        </div>
    </div>
</div>
...
<?php if (!empty($contrato['ubicacion_lat']) && !empty($contrato['ubicacion_lng'])): ?>
<!-- Leaflet.js para el mapa del contrato -->
<link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css"/>
<script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
<script>
document.addEventListener('DOMContentLoaded', function() {
    var lat = <?php echo (float)$contrato['ubicacion_lat']; ?>;
    var lng = <?php echo (float)$contrato['ubicacion_lng']; ?>;
    var mapa = L.map('mapaVerContrato', { zoomControl: true })
        .setView([lat, lng], 16);
    ...
    L.marker([lat, lng])
        .addTo(mapa)
        .bindPopup('Ubicación del contrato')
        .openPopup();
});
</script>
<?php endif; ?>
