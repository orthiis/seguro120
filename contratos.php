    ...
    try {
        ...
        switch ($_POST['action']) {
            ...
            case 'crear':
                ...
                $ubicacion_lat = isset($_POST['ubicacion_lat']) && $_POST['ubicacion_lat'] !== ''
                                 ? (float)$_POST['ubicacion_lat'] : null;
                $ubicacion_lng = isset($_POST['ubicacion_lng']) && $_POST['ubicacion_lng'] !== ''
                                 ? (float)$_POST['ubicacion_lng'] : null;
                $ubicacion_ref = isset($_POST['ubicacion_ref'])
                                 ? trim(htmlspecialchars($_POST['ubicacion_ref'])) : null;
                ...
                $conn->prepare("
                    INSERT INTO contratos
                        (numero_contrato,cliente_id,plan_id,fecha_inicio,fecha_fin,
                         ...
                         ubicacion_lat,ubicacion_lng,ubicacion_ref,
                         telefono1,telefono2,no_whatsapp,direccion,cobrador_id)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                ")->execute([
                    $num,
...
$cobradores = $conn->query("
    SELECT id, codigo, nombre_completo FROM cobradores WHERE estado='activo' ORDER BY codigo
...
$sql = "
    SELECT c.*,
           ...
           c.telefono1      AS contrato_telefono1,
           c.telefono2      AS contrato_telefono2,
           c.no_whatsapp    AS contrato_whatsapp,
                        ...
                        <div class="tbl-actions">
                            ...
                            <button class="btn-tbl edit" title="Editar"
                                    onclick='editarContrato(<?php echo json_encode([
                                        ...
                                        'cobrador_id'      => $ct['cobrador_id'],
                                        'notas'            => $ct['notas'],
                                        'ubicacion_lat'    => $ct['ubicacion_lat'],
                                        'ubicacion_lng'    => $ct['ubicacion_lng'],
                                        'ubicacion_ref'    => $ct['ubicacion_ref'],
                                        'telefono1'        => $ct['contrato_telefono1'],
                                        'telefono2'        => $ct['contrato_telefono2'],
                                        'no_whatsapp'      => $ct['contrato_whatsapp'],
                                        'direccion'        => $ct['contrato_direccion'],
    ...
    <div class="modal-box xl">
        ...
        <div class="mbody">
            <form id="formContrato" method="POST">
                ...
                <!-- ── Cobro y vendedor ── -->
                <div class="fsec-title" style="margin-top:18px;"><i class="fas fa-phone-alt"></i> Contacto</div>
                <div class="form-grid cols-3" style="margin-top:14px;">
                    <div class="form-group">
                        <label class="form-label required" for="ct_telefono1">Teléfono Principal</label>
                        <input type="text" name="telefono1" id="ct_telefono1" class="form-control"
                               required placeholder="809-000-0000">
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="ct_telefono2">Teléfono 2</label>
                        <input type="text" name="telefono2" id="ct_telefono2" class="form-control"
                               placeholder="Opcional">
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="ct_no_whatsapp">No. WhatsApp</label>
                        <input type="text" name="no_whatsapp" id="ct_no_whatsapp" class="form-control"
                               placeholder="809-000-0000">
                    </div>
                </div>
                <div class="form-group" style="margin-top:14px;">
                    <label class="form-label required" for="ct_direccion">Dirección</label>
                    <textarea name="direccion" id="ct_direccion" class="form-control" rows="2"
                              required placeholder="Dirección completa del asegurado"></textarea>
                </div>
                ...
                <div class="form-grid cols-2">
                    ...
                    <div class="form-group">
                        <label class="form-label" for="ct_cobrador_id">Cobrador</label>
                        <select name="cobrador_id" id="ct_cobrador_id" class="form-control">
                ...
                <!-- ── Ubicación del Contrato (CREAR) ── -->
                <div id="mapaSeccionCrear">
                    <div class="fsec-title" style="margin-top:18px;">
                        <i class="fas fa-map-marker-alt"></i> Ubicación del Contrato
                    </div>
                    <div class="form-group">
                        <label class="form-label" for="ubicacion_ref">
                            Referencia de Ubicación
                        </label>
                        <input type="text" id="ubicacion_ref" name="ubicacion_ref"
                               class="form-control"
                    ...
                    <div class="form-group">
                        ...
                        <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap;">
                            <button type="button" class="btn btn-secondary btn-sm"
                                    onclick="centrarMapaEnMiUbicacion('mapaContrato')">
                                <i class="fas fa-location-crosshairs"></i> Mi ubicación
                            </button>
                            <button type="button" class="btn btn-secondary btn-sm"
                                    onclick="limpiarPinMapa('mapaContrato')">
                                <i class="fas fa-trash-alt"></i> Quitar pin
                            </button>
                        </div>
                        <div id="mapaContrato"
                             style="height:280px;border-radius:var(--radius);
                        ...
                        <div id="coordsInfo"
                             style="font-size:11px;color:var(--text-muted);margin-top:4px;min-height:16px;">
                        </div>
                    </div>
                </div>

                <!-- ── Ubicación del Contrato (EDITAR) ── -->
                <div id="mapaSeccionEditar" style="display:none;">
                    <div class="fsec-title" style="margin-top:18px;">
                        <i class="fas fa-map-marker-alt"></i> Ubicación del Contrato
                    </div>
                    <div class="form-group">
                        ...
                        <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap;">
                            <button type="button" class="btn btn-secondary btn-sm"
                                    onclick="centrarMapaEnMiUbicacion('mapaContratoEdit')">
                                <i class="fas fa-location-crosshairs"></i> Mi ubicación
                            </button>
                            <button type="button" class="btn btn-secondary btn-sm"
                                    onclick="limpiarPinMapa('mapaContratoEdit')">
                                <i class="fas fa-trash-alt"></i> Quitar pin
                            </button>
                        </div>
                        <div id="mapaContratoEdit"
                             style="height:280px;border-radius:var(--radius);
                                    border:1px solid var(--border);"></div>
                        <div id="coordsInfoEdit"
                             style="font-size:11px;color:var(--text-muted);margin-top:4px;min-height:16px;">
                        </div>
                    </div>
                </div>
...
function renderVerContrato(d) {
    ...
    /* Bloque ubicación del contrato (si tiene coordenadas) */
    if (d.ubicacion_lat && d.ubicacion_lng) {
        var mapVId = 'mapaVistaContrato_' + d.id;
        html += '<div class="view-block"><div class="view-block-header">' +
            '<div class="view-block-icon" style="background:var(--blue-100,#EFF6FF);color:var(--blue-600,#1D4ED8);">' +
            '<i class="fas fa-map-marker-alt"></i></div>' +
            '<div><div class="view-block-title">Ubicación del Contrato</div></div></div>' +
            '<div class="view-block-body">';
        if (d.ubicacion_ref) {
            html += '<div style="font-size:12.5px;color:var(--gray-600);margin-bottom:10px;">' +
                '<i class="fas fa-info-circle" style="margin-right:5px;color:var(--gray-400);"></i>' +
                esc(d.ubicacion_ref) + '</div>';
        }
        html += '<div id="' + mapVId + '" style="height:220px;border-radius:var(--radius);' +
            'border:1px solid var(--border,#e2e8f0);margin-bottom:10px;"></div>' +
            '<div style="display:flex;gap:8px;flex-wrap:wrap;">' +
            '<a href="https://www.google.com/maps?q=' + d.ubicacion_lat + ',' + d.ubicacion_lng + '" ' +
            'target="_blank" class="btn btn-secondary btn-sm">' +
            '<i class="fas fa-map"></i> Google Maps</a>' +
            '<a href="https://waze.com/ul?ll=' + d.ubicacion_lat + ',' + d.ubicacion_lng + '&navigate=yes" ' +
            'target="_blank" class="btn btn-secondary btn-sm">' +
            '<i class="fas fa-route"></i> Waze</a>' +
            '</div></div></div>';

        setTimeout(function() {
            var elM = document.getElementById(mapVId);
            if (!elM || elM._leaflet_id) return;
            var mapaVista = L.map(mapVId, { zoomControl: true, dragging: false })
                .setView([parseFloat(d.ubicacion_lat), parseFloat(d.ubicacion_lng)], 16);
            L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                attribution: '© OpenStreetMap'
            }).addTo(mapaVista);
            L.marker([parseFloat(d.ubicacion_lat), parseFloat(d.ubicacion_lng)])
                .addTo(mapaVista);
        }, 400);
    }

    /* Bloque contacto del contrato */
    html += '<div class="view-block"><div class="view-block-header">' +
        '<div class="view-block-icon" style="background:var(--green-100,#DCFCE7);color:var(--green-700,#15803D);">' +
        '<i class="fas fa-phone-alt"></i></div>' +
        '<div><div class="view-block-title">Contacto del Contrato</div></div></div>' +
        '<div class="view-block-body"><div class="info-grid cols-3">' +
        infoItem('Teléfono Principal', esc(d.telefono1)) +
        infoItem('Teléfono 2', esc(d.telefono2)) +
        infoItem('No. WhatsApp', esc(d.no_whatsapp)) +
...
function abrirModalNuevo() {
    ...
    /* Mostrar sección mapa creación, ocultar edición */
    document.getElementById('mapaSeccionCrear').style.display  = '';
    document.getElementById('mapaSeccionEditar').style.display = 'none';
    ...
    document.getElementById('ubicacion_ref').value  = '';
    ...
    setTimeout(function() {
        inicializarMapaContrato('mapaContrato', null, null);
    }, 300);
}
...
function editarContrato(ct) {
    ...
    document.getElementById('ct_telefono1').value   = ct.telefono1   || '';
    document.getElementById('ct_telefono2').value   = ct.telefono2   || '';
    document.getElementById('ct_no_whatsapp').value = ct.no_whatsapp || '';
    document.getElementById('ct_direccion').value   = ct.direccion   || '';
    document.getElementById('ct_cobrador_id').value = ct.cobrador_id || '';

    /* Llenar campos de ubicación */
    document.getElementById('ubicacion_ref').value = ct.ubicacion_ref || '';
    document.getElementById('ubicacion_lat').value = ct.ubicacion_lat || '';
    document.getElementById('ubicacion_lng').value = ct.ubicacion_lng || '';

    /* Mostrar sección mapa edición, ocultar creación */
    document.getElementById('mapaSeccionCrear').style.display  = 'none';
    document.getElementById('mapaSeccionEditar').style.display = '';
    ...
    /* Inicializar mapa de edición con pin existente */
    setTimeout(function() {
        inicializarMapaContrato(
            'mapaContratoEdit',
            parseFloat(ct.ubicacion_lat) || null,
            parseFloat(ct.ubicacion_lng) || null
        );
    }, 300);
}
...
/* ═══════════════════════════════════════════════════════════
   MAPAS INTERACTIVOS — CONTRATOS (Leaflet.js)
═══════════════════════════════════════════════════════════ */
var _mapasContratos = {};

function inicializarMapaContrato(mapId, latInicial, lngInicial) {
    /* Destruir instancia previa si existe */
    if (_mapasContratos[mapId]) {
        _mapasContratos[mapId].remove();
        _mapasContratos[mapId] = null;
    }
    var defaultLat  = latInicial || 18.4861;
    var defaultLng  = lngInicial || -69.9312; /* Santo Domingo, RD */
    var defaultZoom = latInicial ? 16 : 13;

    var mapa = L.map(mapId).setView([defaultLat, defaultLng], defaultZoom);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    ...
    if (latInicial && lngInicial) {
        markerPin = L.marker([latInicial, lngInicial], { draggable: true }).addTo(mapa);
        markerPin.on('dragend', function() {
            var pos = markerPin.getLatLng();
            actualizarCoordsInput(mapId, pos.lat, pos.lng);
        });
        actualizarCoordsInput(mapId, latInicial, lngInicial);
    }
    ...
    mapa.on('click', function(e) {
        ...
        if (markerPin) {
            markerPin.setLatLng([lat, lng]);
        } else {
            markerPin = L.marker([lat, lng], { draggable: true }).addTo(mapa);
            markerPin.on('dragend', function() {
                var pos = markerPin.getLatLng();
                actualizarCoordsInput(mapId, pos.lat, pos.lng);
            });
        }
        actualizarCoordsInput(mapId, lat, lng);
    });

    _mapasContratos[mapId] = mapa;
    _mapasContratos[mapId + '_getMarker'] = function() { return markerPin; };
    _mapasContratos[mapId + '_setMarker'] = function(m) { markerPin = m; };

    /* Invalidar tamaño (por si el modal estaba oculto) */
    setTimeout(function() { mapa.invalidateSize(); }, 200);
}

function actualizarCoordsInput(mapId, lat, lng) {
    document.getElementById('ubicacion_lat').value = lat.toFixed(8);
    document.getElementById('ubicacion_lng').value = lng.toFixed(8);
    var infoId = (mapId === 'mapaContrato') ? 'coordsInfo' : 'coordsInfoEdit';
    var el = document.getElementById(infoId);
    if (el) {
        el.innerHTML = '<i class="fas fa-check-circle" style="color:#16A34A;"></i> ' +
            'Pin colocado: ' + lat.toFixed(6) + ', ' + lng.toFixed(6);
    }
}

function centrarMapaEnMiUbicacion(mapId) {
    if (!navigator.geolocation) return;
    navigator.geolocation.getCurrentPosition(function(pos) {
        var mapa = _mapasContratos[mapId];
        if (!mapa) return;
        mapa.setView([pos.coords.latitude, pos.coords.longitude], 16);
    });
}

function limpiarPinMapa(mapId) {
    var mapa = _mapasContratos[mapId];
    if (!mapa) return;
