...
try {
    $cobradores = $conn->query("
        SELECT c.id, c.codigo, c.nombre_completo,
               ...
               (SELECT COUNT(DISTINCT ct2.cliente_id) FROM contratos ct2
                WHERE ct2.cobrador_id = c.id AND ct2.estado = 'activo') AS total_clientes
        FROM cobradores c
...
if ($cobradorSel > 0) {
    ...
    if ($cobradorSelData) {
        ...
        try {
            $s = $conn->prepare("
                SELECT m.*,
                       ...
                       COALESCE(ct_v.direccion, cl.telefono1) AS cv_dir,
                       cl.id        AS cv_id_real
                FROM cobrador_mensajes m
                JOIN usuarios u ON m.usuario_id = u.id
                LEFT JOIN clientes cl ON cl.id = m.cliente_visita_id
                LEFT JOIN contratos ct_v ON ct_v.id = (
                    SELECT id FROM contratos
                    WHERE cliente_id = cl.id AND estado = 'activo'
        ...
        // cobrador_id ahora vive en contratos, no en clientes
        try {
            $s = $conn->prepare("
                SELECT cl.id, cl.nombre, cl.apellidos,
                       ct.direccion, ct.telefono1, ct.numero_contrato
                FROM clientes cl
