                ...
                if ($_POST['action'] === 'crear') {
                    ...
                    $conn->prepare("
                        INSERT INTO clientes
                            (codigo,nombre,apellidos,cedula,telefono1,
                             email,fecha_nacimiento,fecha_registro,estado,notas)
                        VALUES (?,?,?,?,?,?,?,?,'activo',?)
                    ")->execute([
                        $codigo,
                    ...
                    $conn->prepare("
                        UPDATE clientes SET
                            nombre=?,apellidos=?,cedula=?,telefono1=?,
                            email=?,fecha_nacimiento=?,
                            fecha_registro=?,notas=?,estado=?
                        WHERE id=?
                    ")->execute([
                        trim($_POST['nombre']),   trim($_POST['apellidos']),
                        trim($_POST['cedula']),   trim($_POST['telefono1']),
...
if ($filtro_vendedor && $filtro_vendedor !== 'all') {
    $where   .= " AND cl.id IN (SELECT DISTINCT c2.cliente_id FROM contratos c2 WHERE c2.vendedor_id = ? AND c2.estado = 'activo')";
        ...
        <form id="clienteForm" method="POST">
            <div class="mbody">
                ...
                <div class="form-grid cols-2" style="margin-top:14px;">
                    ...
                    <div class="form-group" id="estadoGrp" style="display:none;">
...
function renderVerCliente(d) {
    ...
    html += '<div class="view-block"><div class="view-block-header">' +
        ...
        '<div><div class="view-block-title">Contacto</div></div></div>' +
    ...
    /* datos de contacto del contrato activo */
    if (d.contrato_telefono1 || d.contrato_direccion || d.cobrador_nombre || d.vendedor_nombre) {
        html += '<div class="view-block"><div class="view-block-header">' +
            '<div class="view-block-icon" style="background:var(--blue-50,#EFF6FF);color:var(--blue-600,#1D4ED8);">' +
            '<i class="fas fa-file-contract"></i></div>' +
            '<div><div class="view-block-title">Datos del Contrato Activo</div></div></div>' +
            '<div class="view-block-body"><div class="info-grid">' +
            infoItem('Teléfono 1 (Contrato)', esc(d.contrato_telefono1)) +
            infoItem('Teléfono 2 (Contrato)', esc(d.contrato_telefono2)) +
            infoItem('No. WhatsApp', esc(d.contrato_whatsapp)) +
            infoItem('Dirección', esc(d.contrato_direccion)) +
