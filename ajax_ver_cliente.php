...
$stmt = $conn->prepare("
    SELECT c.*,
           -- Datos de contacto del primer contrato activo
           ct_activo.telefono1     AS contrato_telefono1,
           ct_activo.telefono2     AS contrato_telefono2,
           ct_activo.no_whatsapp   AS contrato_whatsapp,
           ct_activo.direccion     AS contrato_direccion,
           cb_activo.nombre_completo AS cobrador_nombre,
           vd_activo.nombre_completo AS vendedor_nombre,
    ...
    FROM clientes c
    LEFT JOIN contratos ct_activo ON ct_activo.id = (
        SELECT id FROM contratos
        WHERE cliente_id = c.id AND estado = 'activo'
        ORDER BY id DESC LIMIT 1
    )
    LEFT JOIN cobradores cb_activo ON cb_activo.id = ct_activo.cobrador_id
    LEFT JOIN vendedores  vd_activo ON vd_activo.id = ct_activo.vendedor_id
    WHERE c.id = ?
");
$stmt->execute([$id]);
