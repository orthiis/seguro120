...
<div class="app-content">
  ...
  <div id="seccionRuta"
    ...
    <div class="empty-state" id="emptyRuta" style="display:none;">
      ...
      <h3>Sin facturas asignadas</h3>
      <p>No tienes facturas asignadas para hoy. Contacta al administrador para recibir asignaciones.</p>
    </div>
...
function crearTarjetaFactura(f) {
    ...
    /* Referencia de ubicación del contrato */
    if (f.ubicacion_ref) {
        var refDiv = document.createElement('div');
        refDiv.style.cssText = 'font-size:11px;color:var(--text-muted);padding:2px 0;';
        refDiv.innerHTML = '<i class="fas fa-info-circle" style="width:14px;"></i> ' +
            escHtml(f.ubicacion_ref);
        body.appendChild(refDiv);
    }

    /* Botón de navegación GPS si tiene coordenadas del contrato */
    if (f.ubicacion_lat && f.ubicacion_lng) {
        var btnUbicBody = document.createElement('button');
        btnUbicBody.className = 'btn btn-secondary btn-sm';
        btnUbicBody.style.cssText = 'width:100%;margin-top:6px;font-size:11px;';
        btnUbicBody.innerHTML = '<i class="fas fa-map-marker-alt"></i> Ver ubicación / Navegar';
        (function(lat, lng, nombre) {
            btnUbicBody.addEventListener('click', function(e) {
                e.stopPropagation();
                navegarConApp(lat, lng, nombre);
            });
        })(f.ubicacion_lat, f.ubicacion_lng,
           ((f.nombre || '') + ' ' + (f.apellidos || '')).trim());
        body.appendChild(btnUbicBody);
    }
    ...
    if (f.telefono1 && String(f.telefono1).trim()) {
        ...
        /* Botón: WhatsApp — usa no_whatsapp con fallback a telefono1 */
        var waLimpio = (f.no_whatsapp && String(f.no_whatsapp).trim())
                       ? String(f.no_whatsapp).replace(/\D/g, '')
                       : telLimpio;
        var nombreCliente = ((f.nombre || '') + ' ' + (f.apellidos || '')).trim();
        var msgWa =
            'Hola ' + nombreCliente + ', le contactamos respecto' +
            ' a su factura del mes ' + (f.mes_factura || '') + '.';

        var btnWa = document.createElement('a');
        btnWa.className = 'btn btn-sm';
        btnWa.style.cssText = 'background:#16a34a;color:#fff;';
        btnWa.href =
            'https://wa.me/1' + waLimpio +
...
function abrirDetalleFactura(facturaId) {
    ...
    fetch('api/get_factura_det.php?id=' + facturaId, { credentials: 'same-origin' })
        ...
        .then(function(txt) {
            ...
            if (f.telefono1 || f.no_whatsapp) {
                var t1wa   = (f.no_whatsapp && String(f.no_whatsapp).trim())
                             ? String(f.no_whatsapp).replace(/\D/g, '')
                             : String(f.telefono1 || '').replace(/\D/g, '');
    ...
    fetch('api/get_ruta.php', { credentials: 'same-origin' })
        ...
        .then(function(d) {
            ...
            clientes.forEach(function(cl, i) {
                /* Usar ubicacion_lat/lng del contrato */
                var lat = parseFloat(cl.ubicacion_lat || cl.lat);
                var lng = parseFloat(cl.ubicacion_lng || cl.lng);

                if (!lat || !lng || isNaN(lat) || isNaN(lng)) return;

                var nombreCompleto = ((cl.nombre || '') + ' ' + (cl.apellidos || '')).trim();
                var montoFmt = 'RD$' + fmtMonto(cl.monto_pendiente || 0);
                var refUbic = cl.ubicacion_ref ?
                    '<br><small style="color:#666;">' + escHtml(cl.ubicacion_ref) + '</small>' : '';
                var direccion = cl.direccion ?
                    '<br><small>' + escHtml(cl.direccion) + '</small>' : '';

                var colorPin = cl.total_facturas > 2 ? '#DC2626' : '#2563EB';
                var iconPin = L.divIcon({
                    className: '',
                    html: '<div style="background:' + colorPin + ';width:22px;height:22px;' +
                          'border-radius:50%;border:3px solid #fff;' +
                ...
                var popupContent =
                    ...
                    '<div style="font-weight:700;font-size:14px;margin-bottom:4px;">' +
                        escHtml(nombreCompleto) + '</div>' +
                    '<div style="font-size:12px;color:#555;margin-bottom:6px;">' +
                        montoFmt + ' · ' + cl.total_facturas + ' factura(s)' +
                        direccion + refUbic +
                    '</div>' +
                    '<div style="display:flex;flex-direction:column;gap:5px;margin-top:8px;">' +
                    '<button onclick="navegarConApp(\'' + lat + '\',\'' + lng + '\',\'' +
                        escHtml(nombreCompleto).replace(/'/g,'\\\'') + '\')" ' +
                        'style="background:#1D4ED8;color:#fff;border:none;border-radius:6px;' +
                        'padding:7px 10px;font-size:12px;cursor:pointer;font-weight:600;' +
                        'display:flex;align-items:center;gap:6px;justify-content:center;">' +
                        '&#128205; Navegar con GPS</button>' +
    ...
    fetch('api/get_ruta.php', { credentials: 'same-origin' })
        ...
        .then(function(d) {
            ...
            clientes.forEach(function(cl, i) {
                ...
                /* Botón GPS si tiene coordenadas del contrato */
                if (cl.ubicacion_lat && cl.ubicacion_lng) {
                    var btnGps = document.createElement('button');
                    btnGps.className = 'btn btn-success btn-sm';
                    btnGps.style.cssText = 'min-height:30px;padding:0 8px;font-size:11px;';
                    btnGps.innerHTML = '<i class="fas fa-directions"></i>';
                    btnGps.title = 'Navegar con GPS';
                    (function(lat, lng, nombre) {
                        btnGps.addEventListener('click', function() {
                            navegarConApp(lat, lng, nombre);
                        });
                    })(cl.ubicacion_lat, cl.ubicacion_lng,
                       ((cl.nombre || '') + ' ' + (cl.apellidos || '')).trim());
                    btns.appendChild(btnGps);
                } else {
                    /* Fallback: Google Maps por dirección */
...
/* ══════════════════════════════════════════════════════
   NAVEGACIÓN GPS — Modal de opciones
══════════════════════════════════════════════════════ */
function escHtml(s) {
...
function navegarConApp(lat, lng, nombreCliente) {
    ...
    overlay.innerHTML =
        ...
        '<div style="font-size:13px;color:#666;margin-top:4px;">' + escHtml(nombreCliente) + '</div>' +
        '</div>' +
        ...
        '</div>' +
        '<button onclick="cerrarNavOverlay()" ' +
    ...
    overlay.addEventListener('click', function(e) {
        if (e.target === overlay) cerrarNavOverlay();
    });
}

function cerrarNavOverlay() {
    var el = document.getElementById('navOverlay');
    if (el) el.remove();
}
</script>
