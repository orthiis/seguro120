...
try {
    /* ── Clientes con facturas pendientes asignadas a este cobrador ── */
    $stmt = $conn->prepare("
        SELECT DISTINCT
            cl.id, cl.nombre, cl.apellidos,
            c.direccion, c.telefono1, c.no_whatsapp,
            c.ubicacion_lat, c.ubicacion_lng, c.ubicacion_ref,
            c.id AS contrato_id, c.numero_contrato
        FROM clientes cl
        ...
        JOIN asignaciones_facturas af ON af.factura_id = f.id
                                     AND af.cobrador_id = ?
    ...
    /* ── Monto pendiente por cliente (solo facturas asignadas) ── */
    $stmtM = $conn->prepare("
    ...
    foreach ($clientes as &$cl) {
        $stmtM->execute([$cl['id'], $cobradorId]);
        ...
        $cl['lat']               = $cl['ubicacion_lat'] ?? null;
        $cl['lng']               = $cl['ubicacion_lng'] ?? null;
        $cl['ubicacion_lat']     = $cl['ubicacion_lat'] ?? null;
        $cl['ubicacion_lng']     = $cl['ubicacion_lng'] ?? null;
        $cl['ubicacion_ref']     = $cl['ubicacion_ref'] ?? null;
        $cl['contrato_id']       = $cl['contrato_id']   ?? null;
        $cl['numero_contrato']   = $cl['numero_contrato'] ?? null;
    }
    unset($cl);
